...
const serverAddress = "https://86e3-35-188-38-216.ngrok-free.app";
let socket;
...
function App() {
  ...
  useEffect(() => {
    ...
    socket.on('process_output', (data) => {
      setProcessMessages(prevMessages => [...prevMessages, data.message]);
    });

    socket.on('process_complete', (data) => {
      setOutputVideoUrl(data.file_url);
      setLoading(false);
    });
          ...
          {outputVideoUrl && (
            <Card sx={{ mt: 4, width: '100%', maxWidth: 600, backgroundColor: '#444', borderRadius: 3 }}>
              <CardContent>
                ...
                <video src={outputVideoUrl} controls style={{ width: '100%', marginTop: '10px' }} />
                <Button
                  href={outputVideoUrl}
